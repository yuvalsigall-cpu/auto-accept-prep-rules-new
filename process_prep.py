...
def main():
    ...
    if units_col is None or prep_col is None:
        raise SystemExit("Couldn't detect units or prep time columns. Columns found: "+", ".join(df.columns.astype(str)))
    ...
    df["units_bucket"] = pd.cut(df["units"], bins=bins, labels=labels, right=True)
